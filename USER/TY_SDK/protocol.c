...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_MAXTEMP_SET,1000); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINITEMP_SET,0); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MAXHUM_SET,100); //VALUE型数据上报;
    ...
    mcu_dp_value_update(DPID_TEMP_PERIODIC_REPORT,1); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUM_PERIODIC_REPORT,1); //VALUE型数据上报;
...
void wifi_test_result(unsigned char result,unsigned char rssi)
{
    ...
    if(result == 0) {
        ...
        if(rssi == 0x00) {
			...
			UsartPrintf(USART1, "tuya_mdev_test Route Not Find\r\n");
        }else if(rssi == 0x01) {
            UsartPrintf(USART1, "Unauthorized module\r\n");//模块未授权
        }
    }else {
        UsartPrintf(USART1, "WiFi test OK!\r\n");//测试成功
